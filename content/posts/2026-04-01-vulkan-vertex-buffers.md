...
preview: /images/101765a2-b709-4f9a-b9f7-28f19107e486.png
draft: false
tags:
    - Vulkan
categories:
...
```
`vkMapMemory` 先把 staging buffer 的内存映射出来，返回一个指针 `data`。此时 `data` 已经指向 staging buffer 的内存了。然后再拷贝: `memcpy(data, indices.data(), bufferSize)` 往 `data` 写数据，实际上就是写入 staging buffer 的内存。

![alt text](/images/101765a2-b709-4f9a-b9f7-28f19107e486.png)